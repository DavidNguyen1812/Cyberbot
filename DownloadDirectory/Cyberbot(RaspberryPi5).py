...
async def phishing_email_scan(ctx, email_content: str, keep_output_secret: Literal["Yes", "No"]):
    ...
    BERTResult, BERTprobability = Prediction(email_content, BERTtokenizer, BERTPhishingModel, "Phishing Emails", 'cpu')
    AllenAIResult, AllenAIprobability = Prediction(email_content, AllenAItokenizer, AllenAIPhishingModel, "Phishing Emails", 'cpu')
                    ...
                    if checkingCleanData(hashedUrl, "URLs"):
                        ...
                        try:
                            ...
                            if testValidURLresponse.status_code in range(400, 500):
                                ...
                                print(f"URL {url} is invalid with status code: {testValidURLresponse.status_code}")
                                addingHashedData(hashedUrl, "URLs", True)
                                await after.reply(f"URL {url} is invalid with status code: {testValidURLresponse.status_code}",suppress_embeds=True)
                                await after.delete()
                                print("Scan Process Finish!\n\n")
                                return
                        except Exception as error:
                            logScanSession(f"{time.ctime(time.time())}\nUser {after.author.name} re-edited message with invalid URL {url} with Error:\n{error}\n\n")
                            print(f"URL {url} is invalid!")
                            addingHashedData(hashedUrl, "URLs", True)
                            await after.reply(f"URL {url} is invalid! Error getting URL: {error}", suppress_embeds=True)
                            await after.delete()
                            print("Scan Process Finish!\n\n")
                            return
                ...
                for url in URLs:
                    ...
                    if checkingCleanData(hashedUrl, "URLs"):
                        ...
                        try:
                            testValidURLresponse = requests.get(url=url, headers=MAINHEADERS)
                            if testValidURLresponse.status_code in range(400, 500):
                                logScanSession(f"{time.ctime(time.time())}\nUser {message.author.name} sent invalid URL {url} with status code: {testValidURLresponse.status_code}\n\n")
                                print(f"URL {url} is invalid with status code: {testValidURLresponse.status_code}")
                                addingHashedData(hashedUrl, "URLs", True)
                                await message.channel.send(f"URL {url} is invalid with status code: {testValidURLresponse.status_code}", suppress_embeds=True)
                                await message.delete()
                                print("Scan Process Finish!\n\n")
                                return
                        except Exception as error:
                            logScanSession(f"{time.ctime(time.time())}\nUser {message.author.name} sent invalid URL {url} with Error:\n{error}\n\n")
                            print(f"URL {url} is invalid!")
                            addingHashedData(hashedUrl, "URLs", True)
                            await message.channel.send(f"URL {url} is invalid! Error getting URL: {error}", suppress_embeds=True)
                            await message.delete()
                            print("Scan Process Finish!\n\n")
                            return
                UrlScanResults = virusTotalURLScan(URLs)
