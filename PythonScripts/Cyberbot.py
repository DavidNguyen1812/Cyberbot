...
async def checking_expired_passwords_clear_dms_with_admins_update_llm_usages():
    ...
    try:
        ...
        await CronTaskLog(f"An Error occured while completing the task: {e}\nStatus: Error\n\n")
            ...
            if time.time() > adminAccount["Current Account Locked Out Period"]:
                if adminAccount["User Email"] == user_email and adminAccount["User Credential"] == hashlib.sha512(f"{password}{ctx.user.id}".encode()).hexdigest():
                    ...
                    if adminAccount["Failed Log In Attempts"] % 7 != 0:
                        ...
                        await asyncio.to_thread(sendEmail, "Admin Account Locked Out", f"Dear user {ctx.user.name},\n\nYou received this email from Cyberbot to notify that your admin account has been locked for 3 hours due too many invalid authentication attempts while using command /remove_admin_account.\nThe current total lock outs associated with your account is: {len(adminAccount['Locked Out History'])}", adminAccount["User Email"])
                ...
                for admin in CyberBotConfigData["Admins"]:
                    if admin["User ID"] == member.id:
                        if ctx.guild.id not in admin["Accessible Servers"]:
                            if await asyncio.to_thread(sendEmail, "New Accessible Server Added", f"The owner {ctx.user.name} of server {ctx.guild.name} with server ID {ctx.guild.id} has granted your Cyberbot Admin Account access to the server", admin["User Email"]) == "Email sent successfully!":
                ...
                if time.time() > adminAccount["Current Account Locked Out Period"]:
                    ...
                    if adminAccount["User Email"] == accountemail and adminAccount["User Credential"] == hashlib.sha512(f"{accountpassword}{ctx.user.id}".encode()).hexdigest():
                        ...
                        if adminAccount["Failed Log In Attempts"] % 7 != 0:
                            ...
                            adminAccount["Current Account Locked Out Period"] = time.time() + 10800
                            adminAccount["Total Locked Out"] = len(adminAccount["Locked Out History"])
                            await asyncio.to_thread(sendEmail, "Admin Account Locked Out", f"Dear user {ctx.user.name},\n\nYou received this email from Cyberbot to notify that your admin account has been locked for 3 hours due too many invalid login attempts.\nThe current total lock outs associated with your account is: {len(adminAccount['Locked Out History'])}", adminAccount["User Email"])
                            await LoggingCommandBeingExecuted(ctx.user.name,f"/admin_log_in\nCommand Status: Denied/Bad Credentials and account has been locked!")
