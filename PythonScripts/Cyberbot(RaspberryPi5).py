...
import time
import asyncio
import r2pipe # Need pip install r2pipe
...
BERTPhishingModel = loadClassifierModel(BERTPHISHINGPATH, "BERT", "Phishing Emails", 'cpu')
AllenAIPhishingModel = loadClassifierModel(ALLENAIPHISHINGPATH, "Allen AI", "Phishing Emails", 'cpu')
BERTPasswordModel = loadClassifierModel(BERTPASSWORDPATH, "BERT", "Password Strength", 'cpu')
AllenAIPasswordModel = loadClassifierModel(ALLENAIPASSWORDPATH, "Allen AI", "Password Strength", 'cpu')
BERTSpamModel = loadClassifierModel(BERTSPAMPATH, "BERT", "Spam Emails", 'cpu')
ALLENAISpamModel = loadClassifierModel(ALLENAISPAMPATH, "Allen AI", "Spam Emails", 'cpu')
    ...
    for account in CyberBotConfigData["Admins"]:
        for serverID in account["Accessible Servers"]:
            ...
            if guild.me.top_role.position < member.top_role.position and member.id != guild.owner.id:
                await guild.owner.send(f"Member {member.name} from the server {guild.name} ID {serverID} that you owned can kick Cyberbot. Please make sure Cyberbot has a higher role than all the members in the server. The reason is if their discord account is compromised, the attacker can kick Cyberbot out and does not need to be an admin to disable Cyberbot scan protection!")
    ...
    for account in CyberBotConfigData["Admins"]:
        if time.time() >= account["Credential Expiration Age"]:
            ...
            sendEmail("Cyberbot admin account password expired",
                      ...
                      f"Please use command /request_password_reset_token and /change_password with Cyberbot to update your password!\n",
                      account["User Email"])
    print(f"Process Finished!\n\n")
...
async def clean_dms_with_admins():
    ...
    for Account in CyberBotConfigData["Admins"]:
        admin = await Cyberbot.fetch_user(Account["User ID"])
        ...
        if fileExt == ".bin":
            if first1MegaBytes.startswith(b'PK'):
                ...
                if b'conectix' in last512Bytes:
                    return ".vhd"
                if b'koly' in last512Bytes or last512Bytes.startswith(b'EFI PART') or first1MegaBytes.startswith(
                        b'EFI PART'):
                    return ".dmg"
...
def virusTotalFileScan(filePath: str):
    ...
    if response.status_code == 200:
        ...
        for attempt in range(10):
            ...
            print(f"[*] Scan not finished yet (status={status}), retrying in 15s...")
            time.sleep(15)
        return "File can't be scanned"
    else:
    ...
    while len(filePath) != 0:
        ...
        for i in range(len(filePath)):
            ...
            if not fileExt.endswith(ARCHIVEFILEFORMATS):
                ...
                if fileExt.endswith((".iso", ".nrg", ".dmg", ".img", ".udf", ".vhd", ".vhdx", ".qcow2", ".vmdk", ".qcow")):
                    TempDiskMountPoint = f"{DOWNLOADINGDIRPATH}{os.path.basename(filePath[i]).split('.')[0]}MountPoint{random.randint(1, 100000)}/"
                    if fileExt.endswith(".dmg"):
                        print("Disk image is .dmg")
                        print("Checking if dmg disk is encrypted...")
                        checkEncryptedDiskFile = subprocess.run(["dmg2img", "-l", filePath[i]], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
                        checkEncryptedDiskFile = checkEncryptedDiskFile.stdout + checkEncryptedDiskFile.stderr
                        if "encrypted" in checkEncryptedDiskFile.lower():
                            print(f"Disk image {os.path.basename(filePath[i])} is encrypted!")
                            return "Encrypted Error"
                        else:
                            print("Disk image is not encrypted!")
                            try:
                                print("Converting dmg disk to img disk...")
                                RawFilePath = f"{DOWNLOADINGDIRPATH}{os.path.basename(filePath[i]).split('.')[0]}.img"
                                subprocess.run(["dmg2img", filePath[i], RawFilePath], check=True)
                            except subprocess.CalledProcessError:
                                print(f"Failed to convert dmg disk to img disk!")
                                return "Disk Image Error"
                            os.remove(filePath[i])
                            filePath[i] = RawFilePath
                    elif fileExt.endswith((".vhd", ".vhdx", ".qcow2", ".vmdk", ".qcow")):
                        print("Disk image is in .vhd, .vhdx, .qcow2, .vmdk, and .qcow category!")
                        print("Checking if disk is encrypted...")
                        checkEncryptedDiskfile = subprocess.run(["qemu-img", "info", filePath[i]], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True, text=True)
                        checkEncryptedDiskfile = checkEncryptedDiskfile.stdout + checkEncryptedDiskfile.stderr
                        if "encrypted: yes" in checkEncryptedDiskfile:
                            ...
                            print("Disk image is not encrypted!")
                            RawFilePath = f"{DOWNLOADINGDIRPATH}{os.path.basename(filePath[i]).split('.')[0]}.img"
                            print(f"Converting disk format to .img disk format using qemu-img...")
                            try:
                        ...
                        print("Disk image is in .img, .iso, .nrg, and .udf category!")
                        print("Checking if disk is encrypted...")
                    ...
                    try:
                        ...
                        print(f"Content extracted to temp directory!")
                    except subprocess.CalledProcessError:
                    ...
                    for dirpath, _, filenames in os.walk(TempDiskMountPoint):
                        for filename in filenames:
                            ...
                            if not hashedData in DuplicatedFileDetection:
                                print(f"{filename} is written to path {mountPoint}{filepath}")
...
async def create_admin_account(ctx, user_email: str):
    ...
    if not accountExist and not emailTaken:
        ...
        if sendEmail("New Cyberbot Admin Account Created",
            ...
            await ctx.followup.send("A new admin account has been created for you! Please check the email you used to registered the account for more details!")
                        ...
                        if time.time() >= adminAccount["Credential Minimum Age"]:
                            ...
                            if custompassword == "True":
                                ...
                                if len(newpassword) > 30:
                                    ...
                                    passwordStrength, probability = Prediction(newpassword, BERTtokenizer, BERTPasswordModel, "Password Strength", 'cpu')
                                    if passwordStrength < 3 and probability > 0.5:
                                        LoggingCommandBeingExecuted(ctx.user.name,f"/change_password\nCommand Status: Denied/BERT model detected password strength at level {passwordStrength} out of 5 levels ranking system with probability value of {probability}")
                                        ...
                                        passwordStrength, probability = Prediction(newpassword, AllenAItokenizer,AllenAIPasswordModel,"Password Strength", 'cpu')
                                        if passwordStrength < 3 and probability > 0.5:
                                            LoggingCommandBeingExecuted(ctx.user.name,f"/change_password\nCommand Status: Denied/Allen AI model detected password strength at level {passwordStrength} out of 5 levels ranking system with probability value of {probability}")
...
async def phishing_email_scan(ctx, email_content: str, keep_output_secret: Literal["Yes", "No"]):
    ...
    BERTPhishingResult, BERTPhishingprobability = Prediction(email_content, BERTtokenizer, BERTPhishingModel, "Phishing Emails", "cpu")
    AllenAIPhishingResult, AllenAIPhishingprobability = Prediction(email_content, AllenAItokenizer, AllenAIPhishingModel, "Phishing Emails", "cpu")
    BERTSpamResult, BERTSpamprobability = Prediction(email_content, BERTtokenizer, BERTSpamModel, "Spam Emails", "cpu")
    AllenAISpamResult, AllenAISpamprobability = Prediction(email_content, AllenAItokenizer, ALLENAISpamModel, "Spam Emails", "cpu")
...
async def on_message(message):
    ...
    if str(message.guild.id) not in CyberBotConfigData["Automation-Mode"] or str(message.guild.id) not in CyberBotConfigData[
        "Silent-Mode"]:
        ...
        elif str(message.guild.id) not in CyberBotConfigData["Silent-Mode"]:
            ...
            if users[str(message.author.id)]["Current File Scan Operation"] + len(message.attachments) <= LIMITFILESCANPERUSER:
                ...
                for attachment in message.attachments:
                    ...
                    if RootFileTrueExt in CYBERBOTSCOPEOFORMATS:
                            ...
                            for dirpath, _, filenames in os.walk(mountPoint):
                                for filename in filenames:
                                    ...
                                    if fileExt in EXECUTABLEFORMATS:
                                        ...
                                        outputFilePath = os.path.join(mountPoint, f"{os.path.basename(filepath).split('.')[0]}_decompiled.c")
                                        try:
                                            decompiledCode = ''
                                    ...
                                    if fileExt in SCRIPTFILEFORMATS:
                                        ...
                                        if not flaggedMalicious:
                                            ...
                                            if GptScanResult.startswith(("True", "true")):
                                                ...
                                                print(f"GPT analyzed the content of being a potential malware!")
                                        ...
                                        if not flaggedMalicious:
                                            ...
                                            if GeminiScanResult.startswith(("True", "true")):
                                                ...
                                                if len(GeminiScanResult) > 1500:
                                                    ...
                                                    await message.reply(
                                                        f"{GEMINIMODEL} scan result for file {attachment.filename} suggested"
                                                        f" a potential malicious file, therefore it will be deleted!",
                                                        file=resultFile)
                                                else:
                                                    await message.reply(
                                                        f"{GEMINIMODEL}: {GeminiScanResult}\n\nThe file"
                                                        f" {attachment.filename} was detected of being a"
                                                        f" potential malicious file, therefore it will be"
                                                        f" deleted!")
                        ...
                        await message.reply(
                            f"The file {attachment.filename} extension is outside of Cyberbot scope of file "
                ...
                try:
                    await message.author.send(
                        f"The limit of how many file scan processes per user is {LIMITFILESCANPERUSER}!\n"
