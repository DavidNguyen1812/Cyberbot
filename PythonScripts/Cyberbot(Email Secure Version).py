"""Cyberbot authentication is following DAC access control, where each member that have admin account with Cyberbot MUST BE granted by the Server Owner to alter the configuration of Cyberbot for the server"""
"""In this version, Cyberbot will store the admin user registered email as a SHA512 hash. This would make the email notification feature be disabled."""
import os
import subprocess
    ...
    for account in CyberBotConfigData["Admins"]:
        for serverID in account["Accessible Servers"]:
            ...
            if guild.me.top_role.position < member.top_role.position and member.id != guild.owner.id:
                ...
                await CronTaskLog(f"{time.ctime(time.time())}\nCronTask: CHECKING SERVER ROLE HIERARCHY\nMember {member.name} can kick Cyberbot from server {guild.name} ID {serverID}. Warning was sent to server owner\nSTATUS: SUCCESS")
    # print("Process finished!\n\n")


@tasks.loop(time=datetime.time(hour=0, minute=0, tzinfo=ZoneInfo("America/New_York")))  # A task every new day
async def daily_check_for_expired_credential_and_update_llm_usages():
    ...
    for account in CyberBotConfigData["Admins"]:
        try:
            if time.time() >= account["Credential Expiration Age"]:
                ...
                user = Cyberbot.get_user(account["User ID"])
                await user.send(f"Your current admin account password has expired!\nPlease use command **/request_password_reset_token** and **/change_password** in the DM channel with Cyberbot to update your password!")
                await CronTaskLog(f"Password for {account["User Email"]} expired. Notification DM message successfully sent!\nStatus: Success\n\n")
        except Exception as e:
            await CronTaskLog(f"Password for {account["User Email"]} expired. Notification DM message sent FAILURE!\nStatus: Error {e}\n\n")
    print(f"Process Finished!\n\n")
    ...
    if currentTime[2] != previousDate:
        await CronTaskLog(f"{time.ctime(time.time())}\nCron Task: DAILY UPDATING LLMMonthlyUsageReport.png\n")
    ...
    if currentTime[4] != previousYear:
        await CronTaskLog(f"{time.ctime(time.time())}\nCron Task: GENERATING LLM USAGE YEARLY REPORT\n")
    ...
    if currentTime[1] != previousMonth:
        await CronTaskLog(f"{time.ctime(time.time())}\nCron Task: RESETTING LLMMonthlyUsage.csv\n")
...
async def on_ready():
    ...
    daily_check_for_expired_credential_and_update_llm_usages.start()
...
def sendEmail(subject: str, content: str, receiver_email: str) -> Literal["Email sent successfully!", "Email sent unsuccessfully!"]:
    ...
    :return: Email sent status of either failed or successes
    """
...
async def CheckPasswordPwned(password: str) -> bool:
    ...
    :param password: Password to be checked
    :return: True if the password already been breached, else False
    """
...
def ArchivesDiskImagesBombAnalysisAndExtraction(filePath: list, mountPoint: str, archiveLayer=0) -> str:
    ...
    CHUNKSIZE = 5000000000  # Read file to RAM content every 5 GB, adjust based on your memory capacity
...
def uncompressedFileStructure(path: str, indent="") -> str:
    """
    Description: Visualizing the uncompressed file structure of an archive/disk file
    :param path: The root directory of the uncompressed content
    :param indent: Indentation level for better visualization
    :return: The uncompressed file structure
    """
...
async def checkingCleanData(hashedData: str, category: Literal["Archive/Disk File", "Executable/Compiled Files", "Script Files", "Document/PDF Files", "Audio Files", "Image Files", "Video Files", "All Extension", "URLs"]) -> bool:
    """
    Description: Look up the clean hash table for any matching clean Hash
    :param hashedData: SHA256 hash of the file
    :param category: Which file extension category?
    :return: Boolean value of whether the file hash is a hit or not
    """
...
async def checkingFlaggedMaliciousData(hashedData: str, category: Literal["Archive/Disk File", "Executable/Compiled Files", "Script Files", "Document/PDF Files", "Audio Files", "Image Files", "Video Files", "All Extension", "URLs"]) -> bool:
    """
    Description: Look up the malicious hash table for any matching clean Hash
    :param hashedData: SHA256 hash of the file
    :param category: Which file extension category?
    :return: Boolean value of whether the file hash is a hit or not
    """
...
async def addingHashedData(hashedData: str, fileExt, malicious: bool, fileCategory='') -> None:
    """
    Description: Adding a new SHA256 hash to the hash table
    :param hashedData: SHA256 hash
    :param fileExt: File extension/URL
    :param malicious: Malicious table or not?
    :param fileCategory: Which file extension category?
    :return: None
    """
...
async def logScanSession(logData: str) -> None:
    """
    Description: Logging an event of a file/url scan
    :param logData: Scan logs
    :return: None
    """
...
def ghidraDecompile(filepath: str, mountPoint: str, filename: str) -> str:
    """
    Description: Using Ghidra analyzeHeadless and Ghidrathon to de-assemble an executatble/binary program
    :param filepath: The filepath to the binary
    :param mountPoint: The path to write the decompiled/source code of the binary
    :param filename: The filename of the binary
    :return: The path of the decompiled/source code of the binary or ERROR
    """
    ...
    try:
        ...
        print("[Ghidra ERROR] Ghidra timed out")
        os.remove(filepath)
        return "ERROR"
    except Exception as e:
...
async def create_admin_account(ctx, user_email: str):
    ...
    await ctx.response.defer(ephemeral=True)
    emailSHA512Hash = hashlib.sha512(user_email.encode()).hexdigest()
    accountExist = False
    emailTaken = False
    for admin in CyberBotConfigData["Admins"]:
        if admin["User ID"] == ctx.user.id:
    ...
    async with OneTimeTokenLock:
        async with aiofiles.open(ONETIMETOKENPATH, "r") as file:
            resetTokens = json.loads(await file.read())
    if emailSHA512Hash in resetTokens["Email Confirmation Token"]:
        ...
        print(f"A valid Email OTP code already sent to email {emailSHA512Hash}\n\n")
    else:
        if not accountExist and not emailTaken:
            ...
            if await asyncio.to_thread(sendEmail, "Email Verification Code", f"Your Cyberbot Admin Account Email Verification Code is {token}\n\nThis code will expire in 10 minutes!\nIf you did not request this, please ignore this message.", user_email) == "Email sent successfully!":
                resetTokens["Email Confirmation Token"][emailSHA512Hash] = [hashlib.sha1(token.encode()).hexdigest(), time.time() + 600]
                async with OneTimeTokenLock:
                    async with aiofiles.open(ONETIMETOKENPATH, "w") as file:
                        await file.write(json.dumps(resetTokens, indent=4))
...
async def confirm_admin_account(ctx, user_email: str, otp: str):
    ...
    if emailSHA512Hash in resetTokens["Email Confirmation Token"]:
        ...
        if resetTokens["Email Confirmation Token"][emailSHA512Hash][0] == hashlib.sha1(otp.encode()).hexdigest() and time.time() < resetTokens["Email Confirmation Token"][emailSHA512Hash][1]:
            if await asyncio.to_thread(sendEmail, "New Cyberbot Admin Account Created",f"A new Cyberbot admin account was created with a default password:\n{defaultPassword}\nIf you want to change your password, use command /request_password_reset_token and /change_password with Cyberbot!", user_email) == "Email sent successfully!":
                del resetTokens["Email Confirmation Token"][emailSHA512Hash]
                async with OneTimeTokenLock:
                    async with aiofiles.open(ONETIMETOKENPATH, "w") as file:
                        await file.write(json.dumps(resetTokens, indent=4))
                CyberBotConfigData["Admins"].append(
                    {"User ID": ctx.user.id,
                     "User Email": emailSHA512Hash,
...
@Cyberbot.tree.command(
    name="remove_admin_account",
    description="Removing a Cyberbot admin account"
)
@app_commands.describe(
    user_email="Please provide an email address associated with your Cyberbot admin account!",
    password="Please provide a password associated with your Cyberbot admin account!"
)
async def remove_admin_account(ctx, user_email: str, password: str):
    print(f"User {ctx.user.name} initiated /remove_admin_account command")
    await ctx.response.defer(ephemeral=True)
    emailSHA512Hash = hashlib.sha512(user_email.encode()).hexdigest()
    idx = 0
    for adminAccount in CyberBotConfigData["Admins"]:
        if adminAccount["User ID"] == ctx.user.id:
            if time.time() > adminAccount["Current Account Locked Out Period"]:
                if adminAccount["User Email"] == emailSHA512Hash and adminAccount["User Credential"] == hashlib.sha512(f"{password}{ctx.user.id}".encode()).hexdigest():
                    ...
                    if adminAccount["Failed Log In Attempts"] % 7 != 0:
                        ...
                        try:
                            await Cyberbot.get_user(adminAccount["User ID"]).send(f"You received this DM from Cyberbot to notify that your admin account has been locked for 3 hours due too many invalid authentication attempts while using command /remove_admin_account.\nThe current total lock outs associated with your account is: {len(adminAccount['Locked Out History'])}")
                        except Exception:
                            pass
                        await LoggingCommandBeingExecuted(ctx.user.name,f"/remove_admin_account\nCommand Status: Denied/Bad Credentials and account has been locked!")
                        await ctx.followup.send("Too many failed authentication attempts! Your admin account has been locked for 3 hours!")
                        print(f"User {ctx.user.name} input too many invalid authentication attempts, initiating admin account lock out!\n\n")
                async with ConfigLock:
                    async with aiofiles.open(CYBERBOTCONFIG, "w") as file:
                        await file.write(json.dumps(CyberBotConfigData, indent=4))
            else:
    ...
    if ctx.user.id == ctx.guild.owner.id:
        if ctx.guild.me.top_role.position < member.top_role.position and member.id != ctx.guild.owner.id:
            ...
            if member.id == ctx.guild.owner.id:
                await LoggingCommandBeingExecuted(ctx.user.name,f"/adding_admins {member}\nCommand Status: Denied/Member is the server owner and already has admin access")
                await ctx.followup.send(f"{member.name} is this server owner and will always have the admin access!")
                    ...
                    if admin["User ID"] == member.id:
                        if ctx.guild.id not in admin["Accessible Servers"]:
                            try:
                                ...
                                await LoggingCommandBeingExecuted(ctx.user.name,f"/adding_admins {member}\nCommand Status: Denied/Error sending notification {e}!")
...
async def removing_admins(ctx, member: discord.Member):
    ...
    if ctx.user.id == ctx.guild.owner.id:
        if member.id == ctx.guild.owner.id:
            await LoggingCommandBeingExecuted(ctx.user.name,f"/removing_admins {member}\nCommand Status: Denied/Member is the server owner, admin access can be removed!")
            await ctx.followup.send(f"{member.name} is this server owner and will always have the admin access!")
            print(f"{member.name} is the server owner!\n\n")
        else:
            for admin in CyberBotConfigData["Admins"]:
...
@app_commands.describe(
    user_email="Please enter the registered email address with your account!"
)
async def request_password_reset_token(ctx, user_email: str):
    ...
    for adminAccount in CyberBotConfigData["Admins"]:
        if ctx.user.id == adminAccount["User ID"]:
            if time.time() > adminAccount["Current Account Locked Out Period"]:
                if adminAccount["User Email"] == emailSHA512Hash:
                    if time.time() >= adminAccount["Credential Minimum Age"]:
                        async with OneTimeTokenLock:
                            async with aiofiles.open(ONETIMETOKENPATH, "r") as file:
                                resetTokens = json.loads(await file.read())
                        if emailSHA512Hash in resetTokens["Password Reset Token"]:
                            ...
                            for i in range(7):
                                token += random.choice(f"ABCDEFGHIJKLNMOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890!@#$%&*_+=")
                            if await asyncio.to_thread(sendEmail, "Cyberbot Password Reset Token", f"Your password reset token is {token}, it will expired in 3 minutes!", user_email) == "Email sent successfully!":
                                resetTokens["Password Reset Token"][emailSHA512Hash] = [hashlib.sha1(token.encode()).hexdigest(), time.time() + 180]
                                async with OneTimeTokenLock:
                                    async with aiofiles.open(ONETIMETOKENPATH, "w") as file:
                                        await file.write(json.dumps(resetTokens, indent=4))
                                await LoggingCommandBeingExecuted(ctx.user.name,f"/request_password_reset_token\nCommand Status: Approved/A reset token has been sent to user email!")
                                await ctx.followup.send(f"Please check your email {user_email}!")
                                print(f"A new reset token has been sent to user {ctx.user.name} via email {emailSHA512Hash}!\n\n")
                            else:
                                await LoggingCommandBeingExecuted(ctx.user.name, f"/request_password_reset_token\nCommand Status: Denied/Cyberbot can not send the password reset OTP to user email!")
                                await ctx.followup.send(f"Cyberbot can't send the password reset OTP code to your account email {emailSHA512Hash}! Please try the command again and ensure that you have not blocked Cyberbot automation email noreplycyberbot7777@gmail.com!")
                    ...
                    await LoggingCommandBeingExecuted(ctx.user.name,f"/request_password_reset_token\nCommand Status: Denied/User provide invalid email address!")
                    await ctx.followup.send(f"Your provided email address does not match the email that you used to register your admin account!")
...
@app_commands.describe(
    passwordresettoken="Please provide a valid password reset token sent to your email",
    accountemail="Please provide a valid email address associated with your admin account",
    custompassword="Create your own password, or let Cyberbot generate and email one to you",
...
async def change_password(ctx, passwordresettoken: str, accountemail: str, custompassword: Literal["False", "True"], newpassword: str = "Default"):
    ...
    await ctx.response.defer(ephemeral=True)
    emailSHA512Hash = hashlib.sha512(accountemail.encode()).hexdigest()
    for adminAccount in CyberBotConfigData["Admins"]:
        if ctx.user.id == adminAccount["User ID"]:
            if time.time() > adminAccount["Current Account Locked Out Period"]:
                if time.time() >= adminAccount["Credential Minimum Age"]:
                    if emailSHA512Hash == adminAccount["User Email"]:
                        async with OneTimeTokenLock:
                            async with aiofiles.open(ONETIMETOKENPATH, "r") as file:
                                resetTokens = json.loads(await file.read())
                        if emailSHA512Hash in resetTokens["Password Reset Token"]:
                            if resetTokens["Password Reset Token"][emailSHA512Hash][0] == hashlib.sha1(passwordresettoken.encode()).hexdigest() and time.time() < resetTokens["Password Reset Token"][emailSHA512Hash][1]:
                                if time.time() >= adminAccount["Credential Minimum Age"]:
                                    del resetTokens["Password Reset Token"][emailSHA512Hash]
                                    async with OneTimeTokenLock:
                                        async with aiofiles.open(ONETIMETOKENPATH, "w") as file:
                                            await file.write(json.dumps(resetTokens, indent=4))
                                    update = True
...
async def admin_log_in(ctx, accountemail: str, accountpassword: str):
    ...
    if str(ctx.channel.type).startswith("private"):
        await ctx.followup.send("/admin_log_in can only be used in server channels!")
                ...
                if time.time() > adminAccount["Current Account Locked Out Period"]:
                    if str(ctx.guild.id) in adminAccount["Current Admin Session Period"]:
                        if time.time() < adminAccount["Current Admin Session Period"][str(ctx.guild.id)]:
                            ...
                            return
                    if adminAccount["User Email"] == emailSHA512Hash and adminAccount["User Credential"] == hashlib.sha512(f"{accountpassword}{ctx.user.id}".encode()).hexdigest():
                        adminAccount["Failed Log In Attempts"] = 0
                        if time.time() < adminAccount["Credential Expiration Age"]:
                            if ctx.guild.id in adminAccount["Accessible Servers"] or ctx.user.id == ctx.guild.owner.id:
                        ...
                        if adminAccount["Failed Log In Attempts"] % 7 != 0:
                            ...
                            adminAccount["Total Locked Out"] = len(adminAccount["Locked Out History"])
                            try:
                                await Cyberbot.get_user(adminAccount["User ID"]).send(f"You received this DM from Cyberbot to notify that your admin account has been locked for 3 hours due too many invalid authentication attempts while using command /admin_log_in.\nThe current total lock outs associated with your account is: {len(adminAccount['Locked Out History'])}")
                            except Exception:
                                pass
                            await LoggingCommandBeingExecuted(ctx.user.name,f"/admin_log_in\nCommand Status: Denied/Bad Credentials and account has been locked!")
...
async def admin_log_out(ctx):
    ...
    await ctx.response.defer(ephemeral=True)
    if str(ctx.channel.type).startswith("private"):
        await ctx.followup.send("/admin_log_out can only be used in server channels!")
...
# Command for Admins and in non-DM with Cyberbot ONLY!
@Cyberbot.tree.command(
    name="viewing_cyberbot_configuration",
    description="Checking Cyberbot current configuration in the server"
)
async def viewing_cyberbot_configuration(ctx):
    print(f"User {ctx.user.name} initiated /viewing_cyberbot_configurationcommand")
    await ctx.response.defer(ephemeral=True)
    if str(ctx.channel.type).startswith("private"):
        await ctx.followup.send("/viewing_cyberbot_configuration can only be used in server channels!")
                        ...
                        for dirpath, _, filenames in os.walk(mountPoint):
                            for filename in filenames:
                                ...
                                if fileExt in SCRIPTFILEFORMATS:
                                    ...
                                    await asyncio.to_thread(txtToPDFConversion,
                                                            fileBytesContent.decode("utf-8", errors="replace"), pdfPath)
                                    ...
                                    if GptScanResult.startswith(("True", "true")):
                                        ...
                                        print(
                                            f"File too large for OpenAI {GPTMODEL} to scan. Proceed to scan by small chunk...")
                                    ...
                                    if not flaggedMalicious:
                                        ...
                                        if GeminiScanResult.startswith(("True", "true")):
                                            ...
                                            flaggedMalicious, GeminiScanResult = await partialLLMSCAT(filepath,
                                                                                                      "Gemini")
                                            if flaggedMalicious:
